...
(signal, reason, confidence) tuple.  The Hive-Mind (see hive_mind.py) blends
their votes, but ``AIStrategy`` keeps a simple blended interface so the rest of
the code (and the original api.py contract) keeps working.
"""
...
def macd(values, fast=12, slow=26, signal=9):
    ...
    # crude signal line via short ema of recent macd proxy
...
class RSIVoter:
    ...
    def vote(self, prices):
        r = rsi(prices, 14)
        if r <= 30:
            return "BUY", f"RSI oversold ({r:.0f})", min(0.9, 0.6 + (30 - r) / 50)
        if r >= 70:
            return "SELL", f"RSI overbought ({r:.0f})", min(0.9, 0.6 + (r - 70) / 50)
...
class MACDVoter:
    ...
    def vote(self, prices):
        ...
        if line > sig and line > 0:
            return "BUY", "MACD bullish", 0.65
        if line < sig and line < 0:
            return "SELL", "MACD bearish", 0.65
...
class BollingerVoter:
    ...
    def vote(self, prices):
        ...
        if lower is None:
            return "HOLD", "BB: warming up", 0.5
        price = prices[-1]
        if price <= lower:
            return "BUY", "BB: lower band", 0.7
        if price >= upper:
            return "SELL", "BB: upper band", 0.7
...
class MomentumVoter:
    ...
    def vote(self, prices):
        if len(prices) < 12:
            return "HOLD", "MOM: warming up", 0.5
        mom = (prices[-1] - prices[-12]) / prices[-12] if prices[-12] else 0
        if mom > 0.003:
            return "BUY", f"MOM +{mom*100:.2f}%", min(0.85, 0.55 + mom * 30)
        if mom < -0.003:
            return "SELL", f"MOM {mom*100:.2f}%", min(0.85, 0.55 + abs(mom) * 30)
        return "HOLD", "MOM flat", 0.5


# ---------------------------------------------------------------------------
# Simple blended strategy (keeps original AIStrategy().analyze() contract)
# ---------------------------------------------------------------------------
class AIStrategy:
